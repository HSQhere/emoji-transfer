...
def get_drive_pose(toolkits, frames, landmarks, save_size=512, crop=True):
    ...
    if isinstance(frames, list): frames = np.stack(frames, axis=0)
    new_frames, new_landmarks = crop_and_resize(frames, landmarks,
                                                save_size=save_size, crop=crop)
    rot_list = []
    trans_list = []
    with tqdm(total=1, desc='HEAD POSE') as pbar:
        # 只取第一帧处理
        drive_rot, drive_trans = toolkits['h3dmm'].forward_params(new_frames[0], new_landmarks[0])
...
def get_drive_expression_pd_fgc(toolkits, images, landmarks):
    ...
    with tqdm(total=1, desc='PD_FPG_MOTION') as pbar:
        frame = images[0]
