...
from emoji_transfer.utils import (get_drive_pose,
                             get_drive_expression_pd_fgc,
                             det_landmarks,
                             gen_control_heatmaps,
                             load_face_toolkits,
                             append_pipline_weights)
from emoji_transfer import HM5ImagePipeline

from emoji_transfer.tools.sr import RealESRGANer
from emoji_transfer.tools.utils import creat_model_from_cloud

cur_dir = osp.dirname(osp.abspath(__file__))

config_path = osp.join(cur_dir, 'emoji_transfer', 'model_config.json')
...
class Generator(object):
    def __init__(self, gpu_id=0, dtype=torch.float16, pipeline_dict_len=1, sr=True, modelscope=False):
    ...
    @torch.no_grad()
    def load_pipeline(self, type, checkpoint_path, vae_path=None, lora_path=None, lora_scale=1.0, stylize='x1', version='v5'):
